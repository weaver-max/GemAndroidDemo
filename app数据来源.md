...
本文结论均为实测，环境：`gemstone 2.114.10` (AAR) · 2026-09-14
...
```kotlin
val provider = NativeProvider()      // 你只需要写一个

GemGateway(provider, …)
GemSwapper(provider)
GemServiceStatus(provider)
```
...
选 Room 的关键原因是**响应式查询**（DAO 返回 `Flow`） —— 数据一变 UI 自动刷新。
...
*本文档由 AI 辅助整理，端点列表取自 gem 主 App 的 `android/data/services/remote-gem` 实际实现，
